...
- [الإصدار 1.6.0 - نظام إشعارات البريد وتكامل السجلات](#الإصدار-160---نظام-إشعارات-البريد-وتكامل-السجلات)
- [الإصدار 1.5.0 - نظام تحليل المخاطر والبيانات اللحظية](#الإصدار-150---نظام-تحليل-المخاطر-والبيانات-اللحظية)
...
## الإصدار 1.6.0 - نظام إشعارات البريد وتكامل السجلات

**تاريخ الإصدار:** 2026-01-25 (تحديث نهائي)

### المضاف

- **نظام إشعارات البريد الإلكتروني (Email Notification System):** وحدة جديدة `scripts/email_notifier.py` لإرسال تنبيهات فورية عند اكتشاف ثغرات خطيرة.
- **تكامل بريد المستخدم:** تعيين `fcab8090@gmail.com` كبريد افتراضي للتنبيهات في كافة إعدادات النظام ولوحة التحكم.
- **محلل السجلات المتقدم (Advanced Log Analyzer):** سكربت `scripts/log_analyzer.py` لفحص سجلات النظام واكتشاف محاولات الدخول الفاشلة وهجمات الحقن.
- **تحديث الإعدادات:** تفعيل قسم البريد الإلكتروني في `docs/configuration.md` وتحديث واجهة المستخدم لتعكس البريد الجديد.

---

## الإصدار 1.5.0 - نظام تحليل المخاطر والبيانات اللحظية

**تاريخ الإصدار:** 2026-01-25 (تحديث إضافي)

### المضاف

- **نظام تحليل المخاطر (Risk Scoring System):** محرك جديد يحسب درجة أمان المشروع من 100 بناءً على عدد وخطورة الثغرات المكتشفة.
- **مؤشر الحالة الصحية (Health Status):** تصنيف فوري لحالة المشروع (ممتاز، جيد، خطر) يظهر في لوحة التحكم.
- **محاكي البيانات اللحظية (Mock API):** سكربت جديد `scripts/mock_api.py` لتوليد بيانات أمان لحظية لمحاكاة بيئات العمل الحقيقية.
- **تحديث واجهة المستخدم:** إضافة بطاقة تفاعلية لمؤشر المخاطر في الهيدر مع تغيير الألوان ديناميكياً حسب مستوى الخطر.
...
- تصحيح رابط المستودع داخل manifest الناتج.
