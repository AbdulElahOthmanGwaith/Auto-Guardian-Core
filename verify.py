...
class ProjectVerifier:
    ...
    def summary(self):
        """إرجاع ملخص منظم يمكن استهلاكه من CI أو أدوات أخرى."""
        return {
...
https://github.com/AbdulElahOthmanGwaith/Auto-Guardian-Core
...
def main(argv=None):
    """تشغيل التحقق والتغليف مع دعم الاستخدام اليدوي وCI."""
    parser = argparse.ArgumentParser(description="تحقق وتغليف توثيق Auto-Guardian-Core")
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="تشغيل التحقق فقط دون إنشاء ملف ZIP",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="إخراج ملخص JSON مناسب للأدوات وCI",
    )
    parser.add_argument(
        "--output",
    ...
    if args.json:
        # منع رسائل العرض من تلويث مخرجات JSON القابلة للاستهلاك الآلي.
        ...
        if is_valid and not args.check_only:
            ...
            with io.StringIO() as captured, contextlib.redirect_stdout(captured):
                zip_path = packer.create_zip(args.output)
            result["archive"] = str(zip_path) if zip_path else None
            if zip_path is None:
                result["valid"] = False
                result["errors"].append("ZIP creation failed")
    ...
    if args.check_only:
        print_info("تم طلب التحقق فقط؛ لن يتم إنشاء ملف ZIP.")
        return 0
    ...
    print("  2. استخرج الملفات")
    print("  3. انسخ الملفات إلى مجلد Auto-Guardian-Core/")
    print("  4. استخدم: git add . && git commit -m 'docs: إضافة ملفات التوثيق'")
    return 0
